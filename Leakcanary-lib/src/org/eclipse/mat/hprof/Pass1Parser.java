...
public class Pass1Parser extends AbstractParser
{
            ...
            while (curPos < fileSize)
            {
                ...
                if (length < 0)
                    throw new SnapshotException(MessageUtil.format(
                            Messages.Pass1Parser_Error_IllegalRecordLength, in
                                    .position()));
    ...
    private void readStackFrame(long length) throws IOException
    {
        ...
        StackFrame frame = new StackFrame(frameId, lineNr, getStringConstant(methodName),
                getStringConstant(methodSig),
    ...
    private void readClassDump(long segmentStartPos) throws IOException
    {
        ...
        ClassImpl clazz = new ClassImpl(address, className, superClassObjectId,
                classLoaderObjectId, statics, fields);
    ...
    private String getStringConstant(long address)
    {
        ...
        return result == null ? Messages.Pass1Parser_Error_UnresolvedName
                + Long.toHexString(address) : result;
    }
    ...
    private void dumpThreads()
    {
        ...
        try
        {
            ...
            this.monitor.sendUserMessage(Severity.INFO,
                    MessageUtil.format(Messages.Pass1Parser_Info_WroteThreadsTo,
                            outputName), null);
        } catch (IOException e)
        {
